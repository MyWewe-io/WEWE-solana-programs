...
describe('Wewe Token Launch Pad - Integration Tests', () => {
  ...
  it('Launches coin and creates DAMM pool', async () => {
    ...
    const CONFIG_INDEX = new BN(1000);
    const admin = anchor.web3.Keypair.generate();
    await confirm(provider.connection.requestAirdrop(admin.publicKey, 1e9));

    const config = createDammConfig(DAMM_V2_PROGRAM_ID, CONFIG_INDEX);
    await cpAmm.methods
    ...
    const pdas = derivePoolPDAs(program.programId, mint.publicKey, WSOL_MINT, maker.publicKey);
